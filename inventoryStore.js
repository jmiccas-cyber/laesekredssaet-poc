...
async function eksSaveAll() {
  ...
  if (!st.profile.adminCentralId) {
    setInventoryMsg("Vælg først en admin-profil.");
    return;
  }
  const dirtyRows = eksDirtyRows();
  if (!dirtyRows.length) {
    setInventoryMsg("Der er ingen ændringer at gemme.");
    return;
  }
  ...
  for (const tr of dirtyRows) {
    ...
    if (err) {
      setInventoryMsg(`Fejl i række (${rec?.barcode || "ny"}) : ${err}`);
  ...
  setInventoryMsg("Gemmer ændringer...");
  ...
  setInventoryMsg(`Gemte ${payload.length} ændring${payload.length > 1 ? "er" : ""}.`, true);
  await eksPull();
...
async function exportEksToExcel() {
  ...
  if (!ownerId) {
    setInventoryMsg("Vælg først en admin-profil (centralbibliotek) via Skift: Admin ? Booker.");
    return;
  }
  ...
  if (!ExcelHelper) {
    setInventoryMsg("Excel-helper mangler. Prøv at genindlæse siden.");
    return;
  }
  try {
...
async function importEksFromExcel(file) {
  ...
  if (!updates.length && !deletions.length) {
    setInventoryMsg(failures[0] || "Ingen gyldige rækker fundet.");
    return;
  }
  ...
  if (deletions.length) {
    ...
    if (blocking.length) {
      ...
      setInventoryMsg("Eksemplarer kan ikke slettes før tilhørende sæt er nedtaget: " + sample.join(", "));
      return;
    }
  ...
  setInventoryMsg(parts.length ? `Import gennemført: ${parts.join(", ")}.` : "Import gennemført.", true);
  if (failures.length) {
...
async function eksDeleteRow(tr) {
  ...
  if (ownerId && isbn) {
    ...
    if ((available ?? 0) - 1 < reserved) {
      setInventoryMsg(`Eksemplarer for ISBN ${isbn} kan ikke slettes før tilhørende sæt reduceres (krævet: ${reserved}, tilbage efter sletning: ${(available ?? 0) - 1}).`);
      return;
    }
  }
