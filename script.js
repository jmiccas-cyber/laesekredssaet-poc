// Læsekredssæt – v4.0
// - Admin (central) kan oprette/redigere eksemplarer og sæt
// - Eksemplarer: booking_status ∈ {Ledig, Reserveret, Booket}, loan_status ∈ {Ukendt,Hjemme,Udlånt,Reserveret}
// - Region-fanen filtrerer relationer pr. admin-central og kan oprette lånerbiblioteker
...
function msg(selectorOrEl, text, ok=false){
...
function bindTabs(){
  document.querySelectorAll('nav.tabs button[data-tab]').forEach(b=>{
    b.onclick = ()=>{
      ...
      if (b.dataset.tab === 'tab-region') relList();
    };
  });
}
...
function bindSaetControls(){
  ...
  $('#saetNext').onclick = ()=>{
    ...
    if(st.saet.page<m){ st.saet.page++; saetPull(); }
  };
  ...
  rows.forEach(r=>{
    ...
    const btnSave = el('button',{class:'btn primary', onclick: async ()=>{
      const row = {
        ...
        visibility: (vis.value || '').trim().toLowerCase(),
...
async function relList(){
  ...
  (data||[]).forEach(r=>{
    ...
    tr.append(
      ...
      el('td',{}, activeSel),
      el('td',{}, borrowerActiveSel, btnBorrowerSave),
      el('td',{}, btnDel)
    );
    tb.append(tr);
  });
